...
test("满世界：6/6 必填圆满（主角设定为可选维度不计分母），pct=100", () => {
    ...
    // 主角设定为可选维度，单独标记 optional
    assert.equal(r.items.find(i => i.key === "hero").optional, true);
});

test("缺角色/规则/事件/主角设定：done=3，pct=50，评级待充实（<4 按方案）", () => {
    const w = {
        name: "X",
        desc: "有世界观描述",
    ...
    assert.equal(r.pct, Math.round((3 / 6) * 100));
...
test("null / undefined world 兜底：total=6（必填维度）, done=0, pct=0, 评级待充实", () => {
    ...
    assert.equal(computeWorldCompletion(null).total, 6);
});
