...
// ---------- 注册表默认 ----------
test("MODULE_REGISTRY 含 17 个模块且含核心三件套", () => {
    assert.strictEqual(MODULE_REGISTRY.length, 17);
    ...
    for (const must of ["lore", "characters", "memory", "time", "skills", "inventory", "goals",
        ...
        "npc_chat", "world_daily", "web_search"]) {
...
test("sanitizeModules：旧世界（无 modules）补全全部默认模块", () => {
    ...
    assert.strictEqual(Object.keys(out).length, 17);
