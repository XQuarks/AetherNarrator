...
let isCoreLoreCached = false; // P0：模块级变量，buildSystemPrompt 中用于标记核心 lore 是否已缓存
...
3. lore_kb: 知识库对象，包含：
   - ip: 世界名
   - snippets: 数组，每条包含 {id, category（必须覆盖以下类型：规则/世界观/地点/人物/事件/物品/势力/冲突/时间规则）, title, content, keywords（数组）, trigger（仅"事件"类需要，见下）, activation_keys（数组，运行时触发词）, trigger_mode（"keyword"|"regex"|"always"）, scan_depth（数字）, priority（数字，可选，重要度，见下）}
   - "时间规则"类别要求：至少 1 条，描述本世界的时间体系。content 中写明：时代纪元（如"大清乾隆年间"）、历法模式（如"阴历、节气推进"）、时钟粒度（如"分早晨/上午/下午/傍晚/夜晚五个时段"或"具体时钟"）、季节信息。此类别设 trigger_mode="always" 和 priority=5（最高优先级）以确保时间规则始终被注入叙事。activation_keys 可留空或填"时间""时节"等。
