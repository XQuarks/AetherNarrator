...
import { DEFAULT_PERIOD_ORDER, LINK_RELATION_LABELS, STORAGE_KEYS, getBannedConcepts } from "./store.js";
...
function renderLoreReviewBody() {
    ...
    // 使用 _loreSelectedIdx 追踪图谱中选中的节点（默认选第0个）
    if (S._loreSelectedIdx === undefined || S._loreSelectedIdx >= list.length) S._loreSelectedIdx = list.length ? 0 : -1;
    const sel = (S._loreSelectedIdx >= 0 && S._loreSelectedIdx < list.length) ? list[S._loreSelectedIdx] : null;

    // 修订提示
    const revisionHint = S._loreRevisionBuffer
        ? `<div class="lore-warn" style="background:rgba(201,168,124,0.1);border-color:var(--primary);margin-bottom:8px;">
            <strong>AI 修订建议已就绪</strong>（${S._loreRevisionBuffer.length} 条）
            <button class="btn primary" data-action="confirmLoreRevision" style="font-size:12px;padding:2px 10px;margin-left:8px;">✓ 应用</button>
            <button class="btn secondary" data-action="rejectLoreRevision" style="font-size:12px;padding:2px 10px;">✗ 丢弃</button>
           </div>` : "";

    // 图谱 canvas（高度 = list.length 自适应，最少 280px）
    const cH = Math.max(280, Math.min(420, 50 + list.length * 22));
    const spoilerBtn = `<div class="spoiler-toggle" data-action="toggleLoreSpoiler">${S.loreSpoilerHidden ? "🔒 内容已隐藏" : "🔓 已显示全部"}</div>`;

    const editCard = sel ? `
        <div class="lore-edit-card">
            <div class="lore-edit-head">
                <input id="le_title" class="lore-inp" value="${escapeHtml(sel.title || "")}" placeholder="标题" oninput="window._loreDirty=true">
                <input id="le_cat" class="lore-inp lore-cat" value="${escapeHtml(sel.category || "")}" placeholder="类别" oninput="window._loreDirty=true">
                <button class="btn-del" data-action="deleteLoreEntry" data-idx="${S._loreSelectedIdx}">删除</button>
            </div>
            <textarea id="le_content" class="lore-inp lore-content" placeholder="内容" oninput="window._loreDirty=true">${escapeHtml(sel.content || "")}</textarea>
            <div class="lore-row-meta">
                <label>触发词<input id="le_keys" class="lore-inp" value="${escapeHtml((sel.activation_keys || []).join("，"))}" placeholder="逗号分隔" oninput="window._loreDirty=true"></label>
                <label>模式<select id="le_mode" class="lore-inp lore-sel" onchange="window._loreDirty=true">
                    ${["keyword","always","regex"].map(m => `<option value="${m}"${(sel.trigger_mode||"keyword")===m?" selected":""}>${m==="keyword"?"关键词":m==="always"?"常驻":"正则"}</option>`).join("")}
                </select></label>
                <label>优先级<input id="le_pri" class="lore-inp lore-pri" type="number" value="${Number(sel.priority)||0}" oninput="window._loreDirty=true"></label>
            </div>
            ${(sel.links && sel.links.length) ? `<div class="lore-links">关联：${sel.links.map(l => `<span class="lore-link-tag">→ ${escapeHtml(l.target)}（${escapeHtml(LINK_RELATION_LABELS[l.relation]||l.relation)}）</span>`).join(" ")}</div>` : ""}
        </div>` : `<div class="empty-hint" style="padding:12px">点击图谱节点编辑，或点下方"添加条目"</div>`;

    body.innerHTML = spoilerBtn + revisionHint
        + `<canvas id="loreGraphCanvas" style="width:100%;height:${cH}px;border-radius:8px;background:var(--card-bg);cursor:pointer;" title="点击节点选中"></canvas>`
        + editCard
        + `<div style="display:flex;gap:8px;margin-top:8px;">
            <button class="btn secondary" data-action="addLoreEntry" style="font-size:12px;padding:4px 12px;">➕ 添加条目</button>
            <button class="btn primary" data-action="saveLoreReview" style="font-size:12px;padding:4px 12px;">💾 保存</button>
           </div>`;
    setTimeout(() => renderLoreGraphOnPanel(list), 50);
}

// ===== 图谱渲染（B3 面板内） =====
function renderLoreGraphOnPanel(snippets) {
    const canvas = document.getElementById("loreGraphCanvas");
    if (!canvas) return;
    const ctx = canvas.getContext("2d");
    const dpr = window.devicePixelRatio || 1;
    const r = canvas.getBoundingClientRect();
    const W = r.width, H = parseInt(canvas.style.height) || 300;
    canvas.width = W * dpr; canvas.height = H * dpr;
    ctx.setTransform(dpr, 0, 0, dpr, 0, 0);
    const CAT_COLORS = { "规则": "#ffa500", "世界观": "#c9a87c", "地点": "#64c864", "人物": "#6496ff", "事件": "#ff6464", "物品": "#c864ff", "势力": "#ff9646", "冲突": "#ff4646", "时间规则": "#00bcd4" };
    const REL = { causal: "#ff6464", related: "#6496ff", explains: "#64c864", contains: "#c8b464" };

    // 节点：按列排布（3 列网格），无连线的不参与力导向
    const hasLinks = snippets.some(s => s.links && s.links.length);
    const nodes = snippets.map((s, i) => {
        const col = i % 3, row = Math.floor(i / 3);
        return { id: s.id, label: ((s.category||"") + ":" + (s.title||"")).slice(0, 14),
            x: 60 + col * ((W - 80) / 3), y: 30 + row * 55,
            color: CAT_COLORS[s.category] || "#888", vx: 0, vy: 0 };
    });
    ...
    if (hasLinks) {
        const edges = [];
        const id2idx = {};
        nodes.forEach((n, i) => { id2idx[n.id] = i; });
        for (const s of snippets) {
            if (!s.links || !s.links.length) continue;
            for (const l of s.links) {
                if (id2idx[l.target] !== undefined) edges.push({ ai: id2idx[s.id], bi: id2idx[l.target], r: l.relation });
            }
        }
        for (let f = 0; f < 100; f++) {
            ...
            for (const e of edges) {
                ...
                const ff = (d - 70) * 0.02; u.vx += dx/d*ff; u.vy += dy/d*ff; v.vx -= dx/d*ff; v.vy -= dy/d*ff;
            }
            for (let i=0;i<nodes.length;i++) for (let j=i+1;j<nodes.length;j++) {
                const dx=nodes[j].x-nodes[i].x,dy=nodes[j].y-nodes[i].y,d=Math.sqrt(dx*dx+dy*dy);
                if(d<55&&d>0){const ff=(55-d)*0.04;nodes[i].vx-=dx/d*ff;nodes[i].vy-=dy/d*ff;nodes[j].vx+=dx/d*ff;nodes[j].vy+=dy/d*ff;}
            }
            for (const n of nodes) { n.vx*=0.8;n.vy*=0.8;n.x+=n.vx;n.y+=n.vy;n.x=Math.max(30,Math.min(W-30,n.x));n.y=Math.max(16,Math.min(H-16,n.y)); }
        }
    ...
    for (const n of nodes) {
        const sel = (S._loreSelectedIdx >=0 && snippets[S._loreSelectedIdx] && snippets[S._loreSelectedIdx].id === n.id);
        ctx.beginPath();ctx.arc(n.x,n.y,sel?22:18,0,Math.PI*2);
        ctx.fillStyle=sel?"#333":"#1e1e24";ctx.fill();
        ctx.strokeStyle=sel?"#fff":n.color;ctx.lineWidth=sel?2.5:1.5;ctx.stroke();
    ...
    canvas.onclick = e => {
        ...
        const hit = snippets.find((_,i) => Math.hypot(nodes[i].x-mx,nodes[i].y-my)<22);
        if (hit !== undefined) {
            syncLoreSingleFromDOM();
            S._loreSelectedIdx = snippets.indexOf(hit);
            renderLoreReviewBody();
        }
    };
}

// 把当前编辑卡内容写回 _loreEdit 中当前选中项
function syncLoreSingleFromDOM() {
    if (S._loreSelectedIdx < 0 || !S._loreEdit || S._loreSelectedIdx >= S._loreEdit.length) return;
    const el = S._loreEdit[S._loreSelectedIdx];
    const titleEl = document.getElementById("le_title"), catEl = document.getElementById("le_cat"),
          contentEl = document.getElementById("le_content"), keysEl = document.getElementById("le_keys"),
          modeEl = document.getElementById("le_mode"), priEl = document.getElementById("le_pri");
    if (titleEl) el.title = titleEl.value.trim().slice(0,200);
    if (catEl) el.category = catEl.value.trim().slice(0,50);
    if (contentEl) el.content = contentEl.value.trim().slice(0,1000);
    if (keysEl) el.activation_keys = keysEl.value.split(/[，,]/).map(k=>k.trim()).filter(Boolean).slice(0,20);
    if (modeEl) el.trigger_mode = modeEl.value;
    if (priEl) el.priority = Number(priEl.value)||0;
}
...
export function openLoreReview() {
    ...
    S._loreSelectedIdx = S._loreEdit.length ? 0 : -1;
    renderLoreReviewBody();
...
export function editWorldLore(worldId) {
    ...
    S.activeLoreKB = S.currentWorld.lore_kb; // 直接引用世界级KB
    S._loreEditingSaveId = null; // 标记：非存档编辑
...
export function editSaveLore(saveId) {
    ...
    S.currentWorld = world || null;
    // 用存档的知识库副本（如无则从 world 出厂默认深拷贝）
    if (!save.lore_kb) save.lore_kb = (world && world.lore_kb) ? deepClone(world.lore_kb) : { ip: "", snippets: [] };
    if (!Array.isArray(save.lore_kb.snippets)) save.lore_kb.snippets = [];
    S.activeLoreKB = save.lore_kb; // 引用存档级KB
    S._loreEditingSaveId = saveId; // 标记：存档编辑
    openLoreReview();
}
...
export function addLoreEntry() {
    ...
    S._loreEdit.push({
        ...
        keywords: [], activation_keys: [], trigger_mode: "keyword", scan_depth: 1, priority: 0, links: []
    });
    S._loreSelectedIdx = S._loreEdit.length - 1;
    renderLoreReviewBody();
}
...
export function deleteLoreEntry(idx) {
    ...
    if (Array.isArray(S._loreEdit) && i >= 0 && i < S._loreEdit.length) {
        ...
        if (S._loreSelectedIdx >= S._loreEdit.length) S._loreSelectedIdx = S._loreEdit.length - 1;
        renderLoreReviewBody();
    }
}

export async function saveLoreReview() {
    syncLoreSingleFromDOM(); // 先保存当前选中项的编辑
    ...
    // 若编辑的是存档KB，写回存档对象（非 world）
    if (S._loreEditingSaveId) {
        const save = S.saves.find(s => s.id === S._loreEditingSaveId);
        if (save) { save.lore_kb = S.activeLoreKB; saveSaves(); }
    } else {
        saveWorlds(); // 编辑的是世界出厂默认
    }
