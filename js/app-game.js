...
function syncMapPlayerFromNarrative(gameState, changes, prevLocation) {
    ...
    if (map.poi && map.poi.length) {
        for (var i = 0; i < map.poi.length; i++) {
            ...
            if (!pn) continue;
            if (pn === loc || loc.indexOf(pn) >= 0 || pn.indexOf(loc) >= 0) { target = { row: map.poi[i].row, col: map.poi[i].col }; break; }
        }
    }

    // 图例名匹配（兜底）：找瓦片名被 current_location 包含、或包含 current_location 的格子
    if (!target) {
        ...
        if (cands.length === 1) {
            ...
            for (var y = 0; y < map.height && !target; y++) {
                for (var x = 0; x < map.width && !target; x++) {
                    var t = MapDataV1.getTileAt(map, y, x);
                    var ln = (map.tile_legend && map.tile_legend[t] && map.tile_legend[t].name) || '';
                    if (ln === matchName) target = { row: y, col: x };
                }
            }
        }
    }
